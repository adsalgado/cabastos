...
export class UsuarioImagenService {
  ...
  protected convertDateFromClient(usuarioImagen: IUsuarioImagen): IUsuarioImagen {
    const copy: IUsuarioImagen = Object.assign({}, usuarioImagen, {
      fechaAlta: usuarioImagen.fechaAlta && usuarioImagen.fechaAlta.isValid() ? usuarioImagen.fechaAlta.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((usuarioImagen: IUsuarioImagen) => {
        usuarioImagen.fechaAlta = usuarioImagen.fechaAlta ? moment(usuarioImagen.fechaAlta) : undefined;
      });
    }
    return res;
  }
